...
<div id="wrap">
    ...
    <!--PAGE CONTENT -->
    <div id="content">
        <form name="form" action="add-ads.php" method="post" enctype="multipart/form-data">
            <div class="inner">
                <div class="row">
                    <div class="col-lg-12">
                        <h2 class="add-h2">Thêm quảng cáo trái mới</h2>
                    </div>
                </div>
    ...
    /*kiêm tra nút thêm đã được click chưa, nếu được thì tạo 2 biến để lưu dữ liệu*/
{
    ...
    if($_FILES['file']['name']!=""){
        if($_FILES['file']['type']=="image/png"||$_FILES['file']['type']=="image/jpeg"||$_FILES['file']['type']=="image/jpg"){
            if($_FILES['file']['size']>5000000){
                ...
                // dia chi duong dan path den thu muc chua anh
                
                $type=$_FILES['file']['type'];
                $size=$_FILES['file']['size'];
                $tmp_name=$_FILES['file']['tmp_name'];
                move_uploaded_file($tmp_name,$path);
                echo('<script>alert("tai anh thanh cong")</script>');
            }
        }else{
            echo $path;
    ...
    if (isset($add_hinhanh) != '' && ($add_link) != '') {
        include('../host-phpold.php');
        $sqladd = "SELECT * FROM banner_left WHERE vpath_image_banner='" . $add_hinhanh . "'";
        $queryadd = mysql_query($sqladd);
        if (mysql_num_rows($queryadd) != "") {
            echo "<script>alert('Tên đã tồn tại xin vui lòng nhập lại</div>')";
        } else {
            mysql_query("SET NAMES 'UTF8'");
            $sqladd2 = "INSERT INTO banner_left(vpath_image_banner,vlink_image_banner)values('" . $add_hinhanh . "','" . $add_link . "')";
            $queryadd2 = mysql_query($sqladd2);
            echo('<script>location="ads-list.php"</script>');
        }
    } else {
