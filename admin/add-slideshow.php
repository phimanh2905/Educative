...
<div id="wrap">
    ...
    <!--PAGE CONTENT -->
    <div id="content">
        <form name="form" action="add-slideshow.php" method="post" enctype="multipart/form-data">
            <div class="inner">
                <div class="row">
                    <div class="col-lg-12">
                        <h2 class="add-h2">Thêm slideshow mới</h2>
                    </div>
                </div>
    ...
    /*kiêm tra nút thêm đã được click chưa, nếu được thì tạo 2 biến để lưu dữ liệu*/
{
    ...
    if (isset($add_hinhanh) != '' && ($add_link) != '') {
        ...
        $sqladd = "SELECT * FROM slideshow WHERE vpath_image='" . $add_hinhanh . "'";
        $queryadd = mysql_query($sqladd);
        if (mysql_num_rows($queryadd) != "") {
            echo "<script>alert('Tên đã tồn tại xin vui lòng nhập lại</div>')";
        } else {
            mysql_query("SET NAMES 'UTF8'");
            $sqladd2 = "INSERT INTO slideshow(vpath_image,vlink_image)VALUES('" . $add_hinhanh . "','" . $add_link . "')";
            $queryadd2 = mysql_query($sqladd2);
            echo('<script>location="slideshow-list.php"</script>');
        }
    } else {
