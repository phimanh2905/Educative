...
<div id="wrap">
    ...
    <div id="content">
        ...
        <div class="inner">
            <div class="row">
                <div class="col-lg-12">
                    <h2 class="list-product"> Khách hàng </h2>
                </div>
            </div>
            <hr />
            <div class="row">
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        ...
                        $sql_td2 = mysqli_query($connect, "SELECT * FROM khachhang where idkh=".$id);
                        /*sử dụng vòng lặp để tìm kiếm mã môn tên môn theo id bằng check box*/
                        $stt=1;
                        while ($row2 = mysqli_fetch_array($sql_td2)) {
                            /*sử dụng vòng lặp để tìm kiếm mã môn tên môn theo id bằng check box*/
                            echo"<div class='form-group'>
                            <label class='control-label fix_col col-lg-6'>Tên khách hàng: <b>".$row2['tenkh']."</b></label>
                            <label class='control-label fix_col col-lg-6'>Email: <b>".$row2['email']."</b></label>
                            <label class='control-label fix_col col-lg-6'>Số điện thoại: <b>".$row2['sdt']."</b></label>
                            <label class='control-label fix_col col-lg-6'>Địa chỉ : <b>".$row2['diachi']."</b></label>
                        </div>
                        ";
                        }
                            ...
                            <div class="table-responsive">
                                ...
                                <form action="khachhang.php" method="post">
                                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                        ...
                                        include('../host.php');
                                        mysqli_set_charset($connect, "utf8");
                                        $sql_td = mysqli_query($connect, "SELECT * FROM hoadon where idkh=".$id);
                                        /*sử dụng vòng lặp để tìm kiếm mã môn tên môn theo id bằng check box*/
                                        $stt=1;
                                        while ($row = mysqli_fetch_array($sql_td)) {
                                            ...
                                            echo "<td><input name=chk[] class='checkk' type=checkbox value='" . $row['idkh'] . "'></td>";
                                            echo "</tr>";
                                        }
                                        ?>
                                        </tbody>
                                ...
                                if (isset($_POST['btn-delete'])) {
                                    if (empty($_POST['chk'])) {
                                        ...
                                        $del = mysql_query("delete from contact_form where id IN ($id)");
                                        if($del) {echo 'Xoá chuyên mục thành công <script>window.location.href = window.location.pathname;</script>';}
                                            ...
                                            header("Refresh:0; url=send-user-list.php");
                                        }
                                    }
                                }
                                ?>
